...
const DoctorCard: React.FC<DoctorProps> = ({ doctor }) => {
    return <Link href={`/doctors/${doctor.id}`} className="relative w-full sm:!w-[380px] block border border-mainBorderColor rounded-2xl">
        ...
        <Image src={doctor.img} alt={doctor.name} fill className="!static !w-full !h-[350px] sm:!h-[330px] object-cover rounded-t-[15px]"/>
        <div className="px-5 py-[18px]">
            ...
            <Button className="w-full text-[18px]">Подробнее...</Button>
        </div>
    </Link>
}
